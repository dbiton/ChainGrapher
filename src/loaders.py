...
def uncompress_chunk(dset, i):
    ...
    return chunk
def uncompress_chunk_process(dset, i, processpool):
    chunk = dset[i]
    if len(chunk) == 0:
        return []
    chunk = bytes(chunk)
    ...
    try:
        ...
        if os.path.exists(filepath):
            with h5py.File(filepath, 'r') as f:
                ...
                # max_pending = 2
                max_pending = int(os.getenv("LOAD_MAXPENDING", 2))
...
def uncompress_chunk_loading(i, filepath):
    with h5py.File(filepath, 'r') as f_h5:
        ...
        chunk = json.loads(chunk)
        return i, chunk
def load_compressed_file_executor(filepath: str, limit):
    try:
        if os.path.exists(filepath+"_errors.txt"):
            os.remove(filepath+"_errors.txt")
        if os.path.exists(filepath):
            with h5py.File(filepath, 'r') as f:
                ...
                # max_pending = 2
                max_pending = int(os.getenv("LOAD_MAXPENDING", 2))
                with ThreadPoolExecutor(max_pending) as iopool, ProcessPoolExecutor(max_pending) as procpool:
                    futures = [
                        iopool.submit(uncompress_chunk_process, dset, i_chunk, procpool)
                        for i_chunk in range(min(max_pending, chunk_count))
                    ]
                    i_chunk = len(futures)
                    while len(futures) > 0:
